...
public class BookController : Controller
{
    ...
    {
        Book objFromDb = _db.Books.FirstOrDefault(u => u.Book_Id == id);
    ...
    public IActionResult ManageAuthors(int id)
    {
        ...
        List<Author> tempList = _db.Authors.Where(u => !tempListOfAssignedAuthors.Contains(u.Author_Id)).ToList();
    ...
    public IActionResult PlayGround()
    {
        ...
        List<Book> FilteredBook1 = BookList1.Where(b => b.Price > 500).ToList();

        IQueryable<Book> BookList2 = _db.Books;
        List<Book> fileredBook2 = BookList2.Where(b => b.Price > 500).ToList();
        ...
        //VIEWS
        List<BookDetailsFromView> viewList = _db.BookDetailsFromViews.ToList();
        BookDetailsFromView viewList1 = _db.BookDetailsFromViews.FirstOrDefault();
        IQueryable<BookDetailsFromView> viewList2 = _db.BookDetailsFromViews.Where(u => u.Price > 500);

        //RAW SQL

        List<Book> bookRaw = _db.Books.FromSqlRaw("Select * from dbo.books").ToList();

        //SQL Injection attack prone
        var id = 2;
        List<Book> bookTemp1 = _db.Books.FromSqlInterpolated($"Select * from dbo.books where Book_Id={id}").ToList();

        List<Book> booksSproc = _db.Books.FromSqlInterpolated($" EXEC dbo.getAllBookDetails {id}").ToList();

        //.NET 5 only
        List<Book> BookFilter1 = _db.Books.Include(e => e.BookAuthors.Where(p => p.Author_Id == 5)).ToList();
        List<Book> BookFilter2 =
            _db.Books.Include(e => e.BookAuthors.OrderByDescending(p => p.Author_Id).Take(2)).ToList();
